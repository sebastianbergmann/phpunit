...
* Deprecations triggered by `myclabs/deep-copy` are no longer printed when the return value of a depended-upon test is deep-cloned; issues triggered by the code under test while its object graph is copied, in a `__clone()` method for instance, are still reported
