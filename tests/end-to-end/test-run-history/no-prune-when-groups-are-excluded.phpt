--TEST--
A test run that excludes groups does not prune test run history entries of tests it did not run
--FILE--
<?php declare(strict_types=1);
$cacheDirectory = sys_get_temp_dir() . '/phpunit-no-prune-when-groups-are-excluded-' . uniqid();
...
$_SERVER['argv'][] = 'group-that-does-not-exist';
