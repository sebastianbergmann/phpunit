...
{
    foreach (preg_split('/\R/', phpunit($additionalArguments)) as $line) {
...
function listTestsThatDependOnCalculator(): void
{
    ...
    foreach (preg_split('/\R/', phpunit(['--list-tests-that-depend-on', __DIR__ . '/_files/src/Calculator.php'])) as $line) {
