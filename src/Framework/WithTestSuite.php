...
class WithTestSuite extends TestSuite
{
    private $theClass;
    private $method;
    ...
    protected function yieldTests(): Generator
    {
        ...
        try {
            foreach (\PHPUnit\Util\Test::getDataFromTestWithAnnotation($this->theClass->getMethod($this->method)->getDocComment())[0]??[] as $name => $set) {
                if(!is_array($set)) {
