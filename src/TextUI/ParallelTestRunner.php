...
final class ParallelTestRunner
{
    ...
    public function run(Configuration $configuration, TestRunHistory $testRunHistory, TestSuite $suite): void
    {
        try {
            ...
            (new TestSuiteFilterProcessor(Event\Facade::emitter()))->process($configuration, $suite);
    ...
    private function createPhptRunner(Configuration $configuration): PhptRunner
    {
        ...
        return new PhptRunner(new JobRunner($processor, Event\Facade::emitter()), $concurrency);
    }
    ...
    {
        $suite = TestSuite::empty($unit->className(), Event\Facade::emitter());
